...
class AvatarUNRES(nn.Module):
    def __init__(self, pos, vel, acc,
        ...
        self.global_emb = nn.Parameter(torch.randn(3, d_model))
        seq_len = self.N * 3 + 3
        self.rel_pos = nn.Parameter(torch.randn(seq_len, d_model))
    ...
    def forward(self, pos, vel, acc):
        ...
        global_tok = self.global_emb.unsqueeze(0).repeat(B, 1, 1)
        h = torch.cat([global_tok, h + atom_embeddings + self.rel_pos[3:]], dim=1) + self.rel_pos
